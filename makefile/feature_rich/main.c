...
void write_makefile_featurerich()
{
  ...
  char cflags[] = "CXXFLAGS=-Wall -Wextra -g $(foreach D,$(INCDIRS),-I$(D)) "
                  "$(OPT) $(DEPFLAGS)";
  char lflags[] = "LDFLAGS= # math: -lm | ncurses: -lncurses -ltinfo -lmenu";
  ...
  char build_objs_recipe[] = "$(CXX) $(CXXFLAGS) -c -o $@ $<";
  char cleanup[] = "clean:";
